...
class OurDataset(Dataset):
    def __init__(self, data, labels=None, attack_vector=None, multiclass=None):
        ...
        classes = [
            ...
            'PortScan:PortScan - Firewall off',
            'PortScan:PortScan - Firewall on',
            ...
            'DoS / DDoS:DoS Slowhttptest',
            'DoS / DDoS:DoS GoldenEye',
            'DoS / DDoS:DoS Hulk'
        ]
...
def get_nth_split(dataset, n_fold, index, stratify=True):
    print("stratifuy = ", stratify)
    if stratify:
        train_indices, test_indices = next(itertools.islice(StratifiedKFold(
            n_splits=n_fold).split(np.empty(len(attack_vector)), y=attack_vector), index, None))
        ...
        bottom, top = int(math.floor(float(dataset_size)*index/n_fold)
                          ), int(math.floor(float(dataset_size)*(index+1)/n_fold))
        train_indices, test_indices = indices[0:bottom] + \
            indices[top:], indices[bottom:top]
...
class EagerNet(torch.nn.Module):
    def __init__(self, n_input, n_output, n_layers, layer_size):
        ...
        self.beginning = torch.nn.Linear(
            n_input, layer_size+n_output).to(device)
        self.middle = torch.nn.Sequential(
            *[torch.nn.Linear(layer_size, layer_size+n_output).to(device) for _ in range(n_layers)])
        self.end = torch.nn.Linear(layer_size, n_output).to(device)

    def forward(self, x):
        all_outputs = []
        all_xs = []
        output_beginning = self.beginning(x)  # пропускаем через первый слой
        all_xs.append(x)
        # Прогон всех слоев через leaku relu
        # [:,:-self.n_output] ---> извлечение последнего столбца из всех строк.
        x = torch.nn.functional.leaky_relu(
            output_beginning[:, :-self.n_output])

        # добавление выходных нейронов (предсказаний) на 1 слое
...
def train_eager_stopping_nn():
    ...
    ) if opt.multiclass else torch.nn.BCEWithLogitsLoss(reduction="mean")
    n_classes = dataset.labels.shape[-1]
    print("n_classes = ", n_classes)
    net = EagerNet(x.shape[-1], n_classes, opt.nLayers,
                   opt.layerSize).to(device)
    ...
    for i in range(1, sys.maxsize):
        ...
        for data, labels in train_loader:
            ...
            # labels = labels.to(device) if not opt.multiclass else labels.clone().detach()
            labels = labels.to(device) if not opt.multiclass else torch.tensor(
                labels.clone().detach(), dtype=torch.long, device=device).to(device)
            ...
            for output_index, output in enumerate(outputs):
                if opt.eagerStoppingWeightingMethod == "eager_all_one_weights" and output_index < len(outputs)-1:
                    new_output = all_weights[output_index](xs[output_index].detach())[
                        :, -net.n_output:]
                ...
                if not opt.multiclass:
                    print("output detach = ", output.detach())
                    sigmoided_output = torch.sigmoid(output.detach()).squeeze()
                    print("sigmoided_output = ", sigmoided_output)
                    print("tensor size = ", sigmoided_output.size())
                    # time.sleep(100)
                    accuracy = torch.mean(
                        (torch.round(sigmoided_output) == labels.squeeze()).float())
                    writer.add_scalar(
                        f"accuracy_{output_index}", accuracy, samples)

                    confidences = sigmoided_output.detach().clone()
                    confidences[not_attack_mask] = 1 - \
                        confidences[not_attack_mask]
                    writer.add_scalar(
                        f"confidence_{output_index}", torch.mean(confidences), samples)
                    if opt.saveHistogram:
                        writer.add_histogram(
                            f"confidence_hist_{output_index}", confidences, samples)
                else:
            ...
            total_loss = None
            eager_stopping_weight_per_output_per_layer = globals(
            )[opt.eagerStoppingWeightingMethod](losses)
        ...
        torch.save(net.state_dict(), '%s/net_%d.pth' %
                   (writer.logdir, samples))
        torch.save(net, '%s/net_m_%d.pth' % (writer.logdir, samples))
        time.sleep(100)
...
def predict_eager_nn():
    ...
    print("predict_eager_nn = ", test_indices[0])
    if opt.multiclass:
...
def create_binary_plot_eager_(test_indices):
    ...
    for data, labels in test_loader:
        ...
        thing_to_append = torch.sigmoid(torch.stack(
            [output.detach() for output in outputs])).squeeze(2).transpose(1, 0).cpu().numpy()
        thing_to_append = np.maximum(thing_to_append, 1-thing_to_append)
        all_predictions.append(thing_to_append)

    all_predictions = np.concatenate(
        all_predictions, axis=0).astype(np.float32).tolist()
    ...
    accuracy_per_list = [sum([item[output_index] for item in l])
                         for output_index, l in enumerate(lists)]

    print("len(all_predictions)", len(all_predictions),
          "accuracy_per_list", accuracy_per_list)
    ...
    while np.array([len(l) > 0 for l in lists[:n_outputs-1]]).any():
        ...
        accuracy_per_list[candidate_output_index +
                          1] += candidate[candidate_output_index+1]
        ...
        if len(efforts) == 0 or current_lowest >= efforts[-1][0]:
            efforts.append((current_lowest, sum([(output_index+1)*len(lists[output_index]) for output_index in range(
                n_outputs)])/len(all_predictions), sum(accuracy_per_list)/len(all_predictions)))
            efforts_std.append((current_lowest, statistics.stdev(
                [(output_index+1)*len(lists[output_index]) for output_index in range(n_outputs)])/5, statistics.stdev(accuracy_per_list)))
    ...
    things += plt.errorbar(x, y1, yerr=y1err,
                           color=colors[0], label=y1_label, errorevery=100)
...
def multiclass_eager(test_indices, evaluate=False):
    ...
    with torch.no_grad():
        ...
        for data, labels in tqdm(test_loader):
            ...
            for output_index, y_test_pred in enumerate(outputs):
                ...
                # ans = torch.sigmoid(pred)
                # print("pred = ", pred, " ans = ", ans)
                # print("res = ", res)
    ...
    if opt.savePlot:
        ...
        for i, output in enumerate(y_pred_list):
            ...
            cr = confusion_matrix(
                y_list, y_pred_list[i], labels=list(range(n_classes)))
        ...
        clb = plt.colorbar()
        clb.set_label('Точность', rotation=90, fontsize=30)
        plt.yticks(list(range(n_classes)), labels=[
                   dataset.attacks[i] for i in order], fontsize=20)
        plt.xlabel('Слои', fontsize=30)
        plt.ylabel('Классы атак', fontsize=30)
        plt.xticks(list(range(n_outputs)), list(
            range(1, n_outputs+1)), fontsize=20)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataroot', required=True, help='путь к датасету')
    parser.add_argument('--batchSize', type=int, default=128,
                        help='размер выборки данных на каждой итерации')
    parser.add_argument('--nLayers', type=int, default=3,
                        help="количество слоев")
    parser.add_argument('--layerSize', type=int,
                        default=512, help="размер слоя")
    parser.add_argument('--niter', type=int, default=100,
                        help='количество эпох для обучения')
    parser.add_argument('--lr', type=float, default=0.001,
                        help='learning rate')
    parser.add_argument('--dropoutProbability', type=float, default=0.2,
                        help='probability for each neuron to be withheld in an iteration')
    parser.add_argument('--fold', type=int, default=0, help='fold to use')
    parser.add_argument('--nFold', type=int, default=3,
                        help='total number of folds')
    parser.add_argument('--net', default='', help="путь к сохраненной модели")
    parser.add_argument('--function', default='train',
                        help='the function that is going to be called')
    parser.add_argument('--arg', default='', help="optional arguments")
    parser.add_argument('--manualSeed', default=0, type=int,
                        help='seed для перемешивания данных')
    parser.add_argument('--normalizationData', default="",
                        type=str, help='normalization data to use')
    parser.add_argument('--method', choices=['nn'])
    parser.add_argument('--maxRows', default=sys.maxsize, type=int,
                        help='number of rows from the dataset to load (for debugging mainly)')
    parser.add_argument('--maxSize', default=sys.maxsize,
                        type=int, help='only use up to maxSize data samples')
    parser.add_argument('--eagerStoppingWeightingMethod',
                        default="eager_equal_weights", type=str, help="функция взвешивания")
    parser.add_argument('--saveHistogram', action='store_true',
                        help='whether a histogram of confidences is saved for each ')
    parser.add_argument('--optimizer', default="Adam", type=str)
    parser.add_argument('--multiclass', action='store_true')
    parser.add_argument('--savePlot', default='plot.pdf', type=str,
                        help='the name of the figure to store after evaluation')
    parser.add_argument('--saveResults', action='store_true',
                        help='save output results of the main calculation in a numpy npy')
    ...
    # print("suffle_indeces before = ", shuffle_indices)
    ...
    print("attack_vector after = ", attack_vector)
    ...
    if opt.normalizationData == "":
        ...
        stds[stds == 0.0] = 1.0
        print("HREREQ means = ", means, "  stds", stds)
        meanDf = pd.DataFrame(means)
        meanDf.to_csv("means_main.csv", index=False, header=["values"])
        stdDf = pd.DataFrame(stds)
        stdDf.to_csv("std_main.csv", index=False, header=["values"])
    ...
    dataset = OurDataset(x, y, np.expand_dims(
        attack_vector, axis=1), multiclass=True if opt.multiclass else False)
    ...
    if opt.net != '':
        ...
        net = torch.load(opt.net, map_location=device)
...
# Predict
# ./learn.py --dataroot TEST.csv --function predict_eager --method nn --net 'runs/May16_12-32-13_MacBook-Air-Anton.local_0_3/net_m_66447083.pth' --batchSize 1 --layerSize 128 --nLayers 3 --multiclass --maxSize 10000
# Train
# ./learn.py --dataroot CAIA.csv --function train_eager_stopping --method nn --lr 0.001 --batchSize 512 --eagerStoppingWeightingMethod "eager_linearly_increasing_weights" --nLayers 3 --layerSize 128 --multiclass
