...
if __name__ == "__main__":
    ...
    parser.add_argument('--manualSeed', default=0,
                        type=int, help='manual seed')
    ...
    parser.add_argument('--eagerStoppingWeightingMethod', default="eager_equal_weights",
                        type=str, help="how to weight each layer's output when training for eager stopping.")
    ...
    if opt.net == '':
        net = EagerNet(x.shape[-1], n_classes,
                       opt.nLayers, opt.layerSize).to(device)
        print("net", net)
    net = EagerNet(x.shape[-1], n_classes, opt.nLayers,
                   opt.layerSize).to(device)
